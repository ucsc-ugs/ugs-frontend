...
import { getDashboard, getMyExams } from "@/lib/api";
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    // Prefer fetching the user's exams endpoint which uses user_id (auth) to return registrations
    getMyExams()
      .then((res) => {
        if (!mounted) return;
        // Expecting array or object with registeredExams
        setServerData({ registeredExams: res, recentExams: (res || []).filter((r: any) => r.pivot?.attended) });
      })
      .catch((err) => {
        console.warn('getMyExams failed, falling back to dashboard:', err);
        // fallback: try dashboard endpoint
        return getDashboard();
      })
      .then((fallback) => {
        if (!mounted) return;
        if (fallback && !serverData) {
          setServerData(fallback);
        }
      })
      .finally(() => {
                        ...
                        <CardContent className="p-4">
                          <div className="flex gap-3">
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-lg text-foreground leading-tight mb-2">{exam.testName || exam.name || exam.title}</h3>
                              <p className="text-sm text-muted-foreground mb-2">{exam.fullName || exam.description || ''}</p>

                              <div className="mb-2">
                                <p className="text-sm text-muted-foreground font-medium">{exam.university || exam.organization?.name || ''}</p>
                              </div>

                              <div className="mb-2">
                                <p className="text-sm text-foreground">{exam.date || exam.exam_date || exam.examDate || ''}{exam.time ? ` • ${exam.time}` : ''}</p>
                              </div>

                              <div className="mb-2">
                                <span className={`px-2 py-1 text-xs font-medium rounded-full ${exam.paymentStatus === 'Paid' ? 'bg-green-100 text-green-800' : exam.paymentStatus === 'Pending' ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-800'}`}>{exam.paymentStatus || exam.payment_status || 'Unknown'}</span>
                              </div>
                            </div>

                            <img src={exam.image || exam.logo || '/profile_sample.png'} alt={`${exam.university || exam.organization?.name || 'university'} logo`} className="w-20 h-20 rounded-lg object-cover flex-shrink-0" />
                          </div>
                        </CardContent>
                      </Card>
                    </motion.div>
                    ...
                    <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.1 }}>
                      <Card className="hover:scale-[1.02] transition-all duration-200 shadow-sm border-0 bg-card">
                        <CardContent className="p-4">
                          <div className="flex gap-3">
                            <div className="flex-1 min-w-0">
                              <h3 className="font-bold text-lg text-foreground mb-2">{exam.testName || exam.name || exam.title}</h3>
                              <p className="text-sm text-muted-foreground mb-2">{exam.fullName || exam.description || ''}</p>

                              <p className="text-sm text-muted-foreground font-medium mb-3">{exam.university || exam.organization?.name || ''}</p>

                              <div className="mb-4">
                                <span className={`px-2 py-1 text-xs font-medium rounded-full ${exam.resultsOut || exam.results_out ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>{exam.resultsOut || exam.results_out ? 'Results Out' : 'Results Pending'}</span>
                              </div>

                              <div className="flex gap-2">
                                {(exam.resultsOut || exam.results_out) ? (
                                  <button className="w-full bg-primary text-primary-foreground py-2 px-4 rounded-lg font-medium hover:bg-primary/90 transition-colors text-sm">Check Results</button>
                                ) : (
                                  <button className="w-full bg-gray-400 text-white py-2 px-4 rounded-lg font-medium cursor-not-allowed text-sm" disabled>Results Not Available</button>
                                )}
                              </div>
                            </div>
