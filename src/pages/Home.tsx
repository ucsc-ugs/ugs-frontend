...
import { Search, Filter, UserPlus as UserPlusIcon, Loader2, AlertCircle, Mail, X } from "lucide-react";
...
const HomePage = () => {
  ...
  const checkEmailVerification = async () => {
    ...
    try {
      ...
      const response = await axios.get("http://localhost:8000/api/email/verified", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const handleResendVerification = async () => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      if (!token) {
        setResendMessage({ type: 'error', text: 'Authentication required' });
        return;
      }
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-8xl mx-auto p-4 lg:p-6">
        ...
        {isAuthenticated && isEmailVerified === false && showVerificationNotice && (
          <motion.div
