...
const ExamDetailsPage = () => {
  ...
  useEffect(() => {
    const loadExam = async () => {
      ...
      try {
        ...
        if (response.data.exam_dates && response.data.exam_dates.length > 0) {
          setSelectedDateId(response.data.exam_dates[0].id.toString());
        }
      } catch (err: any) {
  ...
  const availableDates = exam.exam_dates?.filter(date => date.status === 'active') || [];
                    ...
                    <RadioGroup value={selectedDateId} onValueChange={setSelectedDateId} className="space-y-3">
                      {availableDates.map((examDate) => (
                        <div key={examDate.id} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
                          <RadioGroupItem value={examDate.id.toString()} id={`date-${examDate.id}`} />
